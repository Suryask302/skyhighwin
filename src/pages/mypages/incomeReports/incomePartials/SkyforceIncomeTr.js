...
import { skyforceIncomeData } from "./TransactionData.js";
import { UncontrolledDropdown, DropdownToggle, DropdownMenu, DropdownItem } from "reactstrap";
import { DataTableHead, DataTableRow, DataTableItem, DataTableBody } from "../../../../components/Component";

const BoostingTr = () => {
    const [data, setData] = useState(skyforceIncomeData);
    const [trans] = useState("");

    useEffect(() => {
        let filteredData;
        if (trans === "Due") {
            filteredData = skyforceIncomeData.filter((item) => item.status === "Due");
        } else if (trans === "Paid") {
            filteredData = skyforceIncomeData.filter((item) => item.status === "Paid");
        } else {
            filteredData = skyforceIncomeData;
        }
